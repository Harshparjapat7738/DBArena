...
/**
 * Translates {@link DomainException} (and Spring's own bean-validation
 * failures) into RFC 7807 {@code application/problem+json}, per root
 * CLAUDE.md conventions. Every other service that exposes HTTP needs the
 * same handler - once a second one exists, lift this into a shared
 * common-web module rather than copy-pasting it (flagged for whichever
 * milestone adds the next HTTP service).
 */
